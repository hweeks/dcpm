...
export const typeSafeExec = async (cmd : string, options: SpawnOptions) : Promise<StandardCmd> => {
  ...
  try {
    ...
    throw new Error(`Command ${cmd} failed with this message: ${error.message}`)
  }
  return logOutput
}

export const runInContext = (cmd : string, options?: SpawnOptions) : Promise<StandardCmd> => {
  const baseOptions = {
    cwd: process.cwd(),
    env: process.env,
    shell: '/bin/sh',
