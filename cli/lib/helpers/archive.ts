...
const archiveSync = (
  ...
  return new Promise((resolve, reject) => {
    ...
    asyncRead(`${pathIn}/manifest.yml`, { encoding: "utf8" }).then(
      (manifestFile) => {
        ...
        const filesToCompress: string[] = [
          ...
          dcpmConfig.config,
          ...Object.entries(dcpmConfig.overrides || {}).map(
            ([_, value]) => value as string
          ),
        ].filter(Boolean);
        const allFiles = [...filesToCompress, ...inputFiles].filter(
          (path) => !path.endsWith("/")
        );
...
const determineFilesToZip = async (pathIn: string) => {
  ...
  return [...finalFiles].filter(
    (file) => Boolean(file) && !ignoreFiles.includes(file)
  );
};
...
export const compressFolder = async (pathIn: string) => {
  ...
  if (!checkForDirectory.isDirectory()) {
    throw new Error(
      "We are zipping up directories here, not files or nothing. Tough luck kiddo."
    );
  }
...
export const decompressToFolder = async (
  pathIn: string,
  destination: string
) => {
  const checkForDirectory = await statSync(pathIn);
  if (!checkForDirectory.isFile()) {
    throw new Error(
      "We are decompressing files here, not directories or nothing. Tough luck kiddo."
    );
  }
