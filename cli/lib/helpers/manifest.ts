...
export const runConfigBuilder = (questions: Question[]) : Promise<Answers> => {
  if (questions.length === 0) {
    throw new Error('It is very hard for me to ask questions without structure, this is not a therapy session.')
  }
  return new Promise((resolve, reject) => {
    inquirer.prompt(questions).then(
      answers => resolve(answers)
    ).catch(err => {
      reject(new Error(`We couldn't ask any questions because of this: ${err.message}`))
    })
  })
}
