...
yargs
  ...
  .command("publish", "publish a version of a compose file", {}, function () {
    publishCommand();
  })
  .command("auth", "login or create a user", {}, function () {
    inquirer
      .prompt([
        {
          type: "input",
  ...
  .command("run <script>", "run a manifest script", {}, function ({ script }) {
  ...
  .command("setup", "run the env setup", {}, function () {
    runSetup();
  })
  .help().argv;
