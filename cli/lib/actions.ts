...
const getReadmeContent = async (fileLocation: string) => {
  ...
  if (fileInfo.isFile()) {
    ...
    throw new Error(
      `There was no readable file at ${filePath}, double check your config.`
    );
  }
};
...
export const loginOrCreateCommand = async (
  username: string,
  password: string
) => {
  const currentConfig = await BuiltConfig.getConfig();
  ...
  try {
    ...
    if (message.includes(username)) {
      ...
      warn(
        message || "We blew up trying to log you in, but I have no idea why."
      );
    }
  }
};

export const modifyPermsCommand = async (
  user: string,
  action: "add" | "remove",
  packageName: string
) => {
...
export const executeCommand = async (script: string) => {
  try {
    ...
    if (manifestInfo.scripts) {
      ...
      if (!possibleScript) {
        throw new Error(
          `I really couldn't find any scripts by the name ${script} in ${cwd}. Maybe you fat fingered it?`
        );
      }
...
export const runSetup = async () => {
  try {
    ...
    if (manifestInfo.env?.length) {
      ...
      const envFileTemplate = await asyncRead(
        `${__dirname}/templates/env.mustache`,
        "utf8"
      );
...
export const buildAdditionalConfigs = async () => {
  try {
    ...
    if (manifestInfo.extraConfigs) {
      const allConfigsToBuild = manifestInfo.extraConfigs.map(
        async (configToBuild) => {
          const answers = await runConfigBuilder(configToBuild.questions);
          const fileTemplate = await asyncRead(
            path.resolve(cwd, configToBuild.template),
            "utf8"
          );
          const builtFile = Mustache.render(fileTemplate, answers);
          await asyncWrite(path.resolve(cwd, configToBuild.output), builtFile);
          log(
            `Alright, we built your file and plopped it here: ${configToBuild.output}`
          );
        }
      );
      ...
      warn(
        "I really can't just come up with questions to ask, we are not using blockchain or AI here, I'm no unicorn."
      );
    }
  } catch (error) {
    const { message } = error;
